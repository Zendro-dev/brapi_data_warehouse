...
module.exports = class season extends Sequelize.Model {
    ...
    static async batchReadById(keys) {
        let queryArg = {
            operator: "in",
            field: season.idAttribute(),
            value: keys.join(),
            valueType: "Array",
        };
        let cursorRes = await season.readAllCursor(queryArg);
        cursorRes = cursorRes.seasons.reduce(
            (map, obj) => ((map[obj[season.idAttribute()]] = obj), map), {}
        );
        return keys.map(
            (key) =>
    ...
    static readByIdLoader = new DataLoader(season.batchReadById, {
        cache: false,
    });
     ...
     * @return {object} The requested record as an object with the type season, or an error object if the validation after reading fails
     * @throws {Error} If the requested record does not exist
     */
    static async readById(id) {
        return await season.readByIdLoader.load(id);
    }
    /**
    ...
    static async readAllCursor(search, order, pagination, benignErrorReporter) {
        ...
        return {
            edges,
            pageInfo,
            seasons: edges.map((edge) => edge.node)
        };
    }
     ...
     */
    static async add_observations_IDs(seasonDbId, observations_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            observations_IDs.forEach(idx => {
                promises.push(models.observation.add_season_ID(idx, seasonDbId, benignErrorReporter, token, false));
            });
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_observations_IDs(seasonDbId, observations_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            observations_IDs.forEach(idx => {
                promises.push(models.observation.remove_season_ID(idx, seasonDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
    ...
    /**
     * base64Encode - Encode  season to a base 64 String
     *
     * @return {string} The season object, encoded in a base 64 String
     */
    base64Encode() {
    ...
    /**
     * stripAssociations - Instance method for getting all attributes of season.
     *
     * @return {object} The attributes of season in object form
     */
    stripAssociations() {
    ...
    /**
     * externalIdsArray - Get all attributes of season that are marked as external IDs.
     *
     * @return {Array<String>} An array of all attributes of season that are marked as external IDs
     */
    ...
    /**
     * externalIdsObject - Get all external IDs of season.
     *
