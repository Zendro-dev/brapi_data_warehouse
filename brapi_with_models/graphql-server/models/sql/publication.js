...
module.exports = class publication extends Sequelize.Model {
     ...
     *
     * @param {Id}   publicationDbId   IdAttribute of the root model to be updated
     * @param {Id}   trial_ID Foreign Key (stored in "Me") of the Association to be updated.
     * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_trial_ID(publicationDbId, trial_ID, benignErrorReporter, token, handle_inverse = true) {
        try {
            //handle inverse association
            if (handle_inverse) {
                await models.trial.add_publications_IDs(trial_ID, [`${publicationDbId}`], benignErrorReporter, token, false);
            }
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_trial_ID(publicationDbId, trial_ID, benignErrorReporter, token, handle_inverse = true) {
        try {
            //handle inverse association
            if (handle_inverse) {
                await models.trial.remove_publications_IDs(trial_ID, [`${publicationDbId}`], benignErrorReporter, token, false);
            }
            let updated = await publication.update({
                trial_ID: null
            }, {
