...
module.exports = class trait extends Sequelize.Model {
    ...
    static async batchReadById(keys) {
        let queryArg = {
            operator: "in",
            field: trait.idAttribute(),
            value: keys.join(),
            valueType: "Array",
        };
        let cursorRes = await trait.readAllCursor(queryArg);
        cursorRes = cursorRes.traits.reduce(
            (map, obj) => ((map[obj[trait.idAttribute()]] = obj), map), {}
        );
        return keys.map(
            (key) =>
    ...
    static readByIdLoader = new DataLoader(trait.batchReadById, {
        cache: false,
    });
     ...
     * @return {object} The requested record as an object with the type trait, or an error object if the validation after reading fails
     * @throws {Error} If the requested record does not exist
     */
    static async readById(id) {
        return await trait.readByIdLoader.load(id);
    }
    /**
    ...
    static async readAllCursor(search, order, pagination, benignErrorReporter) {
        ...
        return {
            edges,
            pageInfo,
            traits: edges.map((edge) => edge.node)
        };
    }
     ...
     */
    static async add_ontologyReference_ID(traitDbId, ontologyReference_ID, benignErrorReporter, token, handle_inverse = true) {
        try {
            //handle inverse association
            if (handle_inverse) {
                await models.ontologyreference.add_trait_ID(ontologyReference_ID, traitDbId, benignErrorReporter, token, false);
            }
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_germplasmAttribute_ID(traitDbId, germplasmAttribute_ID, benignErrorReporter, token, handle_inverse = true) {
        try {
            //handle inverse association
            if (handle_inverse) {
                await models.germplasmattribute.add_trait_ID(germplasmAttribute_ID, traitDbId, benignErrorReporter, token, false);
            }
            let updated = await trait.update({
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_additionalInfo_IDs(traitDbId, additionalInfo_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            additionalInfo_IDs.forEach(idx => {
                promises.push(models.additionalinfo.add_trait_ID(idx, traitDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }

        let record = await super.findByPk(traitDbId);
        if (record !== null) {
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_externalReferences_IDs(traitDbId, externalReferences_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            externalReferences_IDs.forEach(idx => {
                promises.push(models.externalreference.add_trait_ID(idx, traitDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_observationVariables_IDs(traitDbId, observationVariables_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            observationVariables_IDs.forEach(idx => {
                promises.push(models.observationvariable.add_trait_ID(idx, traitDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_ontologyReference_ID(traitDbId, ontologyReference_ID, benignErrorReporter, token, handle_inverse = true) {
        try {
            //handle inverse association
            if (handle_inverse) {
                await models.ontologyreference.remove_trait_ID(ontologyReference_ID, traitDbId, benignErrorReporter, token, false);
            }
            let updated = await trait.update({
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_germplasmAttribute_ID(traitDbId, germplasmAttribute_ID, benignErrorReporter, token, handle_inverse = true) {
        try {
            //handle inverse association
            if (handle_inverse) {
                await models.germplasmattribute.remove_trait_ID(germplasmAttribute_ID, traitDbId, benignErrorReporter, token, false);
            }
            let updated = await trait.update({
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_additionalInfo_IDs(traitDbId, additionalInfo_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            additionalInfo_IDs.forEach(idx => {
                promises.push(models.additionalinfo.remove_trait_ID(idx, traitDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_externalReferences_IDs(traitDbId, externalReferences_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            externalReferences_IDs.forEach(idx => {
                promises.push(models.externalreference.remove_trait_ID(idx, traitDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_observationVariables_IDs(traitDbId, observationVariables_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            observationVariables_IDs.forEach(idx => {
                promises.push(models.observationvariable.remove_trait_ID(idx, traitDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
    ...
    /**
     * base64Encode - Encode  trait to a base 64 String
     *
     * @return {string} The trait object, encoded in a base 64 String
     */
    base64Encode() {
    ...
    /**
     * stripAssociations - Instance method for getting all attributes of trait.
     *
     * @return {object} The attributes of trait in object form
     */
    stripAssociations() {
    ...
    /**
     * externalIdsArray - Get all attributes of trait that are marked as external IDs.
     *
     * @return {Array<String>} An array of all attributes of trait that are marked as external IDs
     */
    ...
    /**
     * externalIdsObject - Get all external IDs of trait.
     *
