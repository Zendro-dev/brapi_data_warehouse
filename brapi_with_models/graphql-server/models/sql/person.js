...
module.exports = class person extends Sequelize.Model {
     ...
     *
     * @param {Id}   personDbId   IdAttribute of the root model to be updated
     * @param {Array}   additionalInfo_IDs Array foreign Key (stored in "Me") of the Association to be updated.
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_additionalInfo_IDs(personDbId, additionalInfo_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            additionalInfo_IDs.forEach(idx => {
                promises.push(models.additionalinfo.add_person_ID(idx, personDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }

        let record = await super.findByPk(personDbId);
        if (record !== null) {
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_externalReferences_IDs(personDbId, externalReferences_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            externalReferences_IDs.forEach(idx => {
                promises.push(models.externalreference.add_person_ID(idx, personDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
    ...
    /**
     * add_lists_IDs - field Mutation (model-layer) for to_many associationsArguments to add
     *
     * @param {Id}   personDbId   IdAttribute of the root model to be updated
     * @param {Array}   lists_IDs Array foreign Key (stored in "Me") of the Association to be updated.
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_lists_IDs(personDbId, lists_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            lists_IDs.forEach(idx => {
                promises.push(models.list.add_listOwnerPerson_ID(idx, personDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
        ...
        let record = await super.findByPk(personDbId);
        if (record !== null) {
            let updated_ids = helper.unionIds(JSON.parse(record.lists_IDs), lists_IDs);
            updated_ids = JSON.stringify(updated_ids);
            await record.update({
                lists_IDs: updated_ids
            });
        }
    }
    /**
     * add_programs_IDs - field Mutation (model-layer) for to_many associationsArguments to add
     *
     * @param {Id}   personDbId   IdAttribute of the root model to be updated
     * @param {Array}   programs_IDs Array foreign Key (stored in "Me") of the Association to be updated.
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_programs_IDs(personDbId, programs_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            programs_IDs.forEach(idx => {
                promises.push(models.program.add_leadPerson_ID(idx, personDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
        ...
        let record = await super.findByPk(personDbId);
        if (record !== null) {
            let updated_ids = helper.unionIds(JSON.parse(record.programs_IDs), programs_IDs);
            updated_ids = JSON.stringify(updated_ids);
            await record.update({
                programs_IDs: updated_ids
            });
        }
    }
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_additionalInfo_IDs(personDbId, additionalInfo_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            additionalInfo_IDs.forEach(idx => {
                promises.push(models.additionalinfo.remove_person_ID(idx, personDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_externalReferences_IDs(personDbId, externalReferences_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            externalReferences_IDs.forEach(idx => {
                promises.push(models.externalreference.remove_person_ID(idx, personDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
    ...
    /**
     * remove_lists_IDs - field Mutation (model-layer) for to_many associationsArguments to remove
     *
     * @param {Id}   personDbId   IdAttribute of the root model to be updated
     * @param {Array}   lists_IDs Array foreign Key (stored in "Me") of the Association to be updated.
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_lists_IDs(personDbId, lists_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            lists_IDs.forEach(idx => {
                promises.push(models.list.remove_listOwnerPerson_ID(idx, personDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
        ...
        let record = await super.findByPk(personDbId);
        if (record !== null) {
            let updated_ids = helper.differenceIds(JSON.parse(record.lists_IDs), lists_IDs);
            updated_ids = JSON.stringify(updated_ids);
            await record.update({
                lists_IDs: updated_ids
            });
    ...
    /**
     * remove_programs_IDs - field Mutation (model-layer) for to_many associationsArguments to remove
     *
     * @param {Id}   personDbId   IdAttribute of the root model to be updated
     * @param {Array}   programs_IDs Array foreign Key (stored in "Me") of the Association to be updated.
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_programs_IDs(personDbId, programs_IDs, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            programs_IDs.forEach(idx => {
                promises.push(models.program.remove_leadPerson_ID(idx, personDbId, benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
        ...
        let record = await super.findByPk(personDbId);
        if (record !== null) {
            let updated_ids = helper.differenceIds(JSON.parse(record.programs_IDs), programs_IDs);
            updated_ids = JSON.stringify(updated_ids);
            await record.update({
                programs_IDs: updated_ids
            });
